...
export async function onRequest(context){
  ...
  try{
    ...
    if(!slug)return json({ok:true,slug:'',available:false,conflicts:[],message:'slug를 입력하세요.'});

    const [pageValues,snapshotValues]=await Promise.all([
      readSheetValues(env,'PLATFORM_PAGES','A:M'),
    ...
    for(const row of pages){
      ...
      if(pageId&&String(row.page_id||'')===pageId)continue;
      conflicts.push({kind:'draft',pageId:String(row.page_id||''),title:String(row.title||''),status:String(row.status||'draft')});
    }
    for(const row of snapshots){
      if(String(row.slug||'')!==slug||String(row.state||'')!=='active')continue;
      if(pageId&&String(row.page_id||'')===pageId)continue;
      conflicts.push({kind:'published',pageId:String(row.page_id||''),snapshotId:String(row.snapshot_id||''),title:String(row.title||''),status:'published'});
    }

    return json({ok:true,slug,available:conflicts.length===0,conflicts,message:conflicts.length?'이미 다른 페이지에서 사용 중입니다.':'사용할 수 있습니다.'});
  }catch(error){
    console.error(error);
