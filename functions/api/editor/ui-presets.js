...
async function listPresets(env){
  const values=await readSheetValues(env,`${SHEET}!A:J`);
  const rows=valuesToObjects(values);
  const presets=rows.filter(row=>row.preset_id&&isKnownUiCapability(row.capability_id)).map(row=>({
...
async function savePresets(env,body){
  ...
  if(presets.length>200)throw new Error('preset 수를 확인해 주세요.');
  const now=koreaTime();
  const values=await readSheetValues(env,`${SHEET}!A:J`);
  const headers=ensureHeaderMap(values[0],['preset_id','capability_id','name','config_json','source','status','created_at','updated_at','notes','version']);
  const existing=new Map();
  for(let i=1;i<values.length;i++){
    const id=String(values[i][headers.preset_id]||'');
    if(id)existing.set(id,{row:values[i],rowNumber:i+1});
  }
  let saved=0;
  for(const input of presets){
    const capabilityId=String(input?.capabilityId||'').trim();
    if(!isKnownUiCapability(capabilityId))continue;
    const id=cleanId(input?.id||`preset_${crypto.randomUUID()}`,180);
    ...
    if(JSON.stringify(config).length>30000)throw new Error(`preset 설정이 너무 큽니다: ${name}`);
    const previous=existing.get(id);
    const createdAt=previous?String(previous.row[headers.created_at]||now):String(input?.createdAt||now);
    const version=Math.max(1,(previous?Number(previous.row[headers.version]||0):Number(input?.version||0))+1);
    const row=[[id,capabilityId,name,JSON.stringify(config),String(input?.source||'user').slice(0,80),String(input?.status||'draft').slice(0,40),createdAt,now,String(input?.notes||'').slice(0,4000),version]];
    if(previous)await updateRange(env,`${SHEET}!A${previous.rowNumber}:J${previous.rowNumber}`,row);
    else await appendRange(env,`${SHEET}!A:J`,row);
    saved+=1;
  }
