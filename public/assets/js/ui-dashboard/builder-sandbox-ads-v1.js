(function(){
  ...
  const DEFAULTS={enabled:false,width:120,height:430,top:148,gap:18,follow:true};
  ...
  function activeContentZone(){
    const frameDoc=doc(),frameWin=win();if(!frameDoc||!frameWin||frameWin.innerWidth<1360)return null;
    const areas=[...frameDoc.querySelectorAll('.chapter .section .content')];
    const minTop=96;
    for(const area of areas){
      const rect=area.getBoundingClientRect();
      if(rect.width<420)continue;
      if(rect.bottom>minTop+240&&rect.top<frameWin.innerHeight-100)return {area,rect};
    }
    return null;
  }

  function placeInGutter(side){
    ...
    const height=Math.max(180,Number(cfg.height)||430);
    const gap=Math.max(0,Number(cfg.gap)||0);
    const preferredTop=Math.max(86,Number(cfg.top)||148);
    const fixedTop=Math.max(preferredTop,rect.top+18);
    const minimumVisible=Math.min(220,height);
    if(rect.bottom<fixedTop+minimumVisible)return;

    const leftSpace=rect.left;
    const rightSpace=frameWin.innerWidth-rect.right;
    const hasRoom=side==='left'?leftSpace>=width+gap+12:rightSpace>=width+gap+12;
    if(!hasRoom)return;

    node.style.width=`${width}px`;
    node.style.height=`${height}px`;
    node.style.setProperty('--sandbox-ad-width',`${width}px`);
    node.style.setProperty('--sandbox-ad-height',`${height}px`);
    node.style.setProperty('--sandbox-ad-gap',`${gap}px`);
    node.style.position=cfg.follow?'fixed':'absolute';
    node.style.top=cfg.follow?`${fixedTop}px`:`${frameWin.scrollY+rect.top+18}px`;
    node.style.bottom='auto';
    if(side==='left'){
      node.style.left='auto';
      node.style.right=`${Math.max(8,frameWin.innerWidth-rect.left+gap)}px`;
    }else{
      node.style.right='auto';
      node.style.left=`${Math.max(8,rect.right+gap)}px`;
    }
    node.dataset.adZoneActive='true';
  }

  function scheduleZone(){
    if(zoneRaf)return;
    zoneRaf=requestAnimationFrame(()=>{zoneRaf=0;placeInGutter('left');placeInGutter('right');});
  }
  ...
  function apply(side){
    ...
    if(strong)strong.textContent=`${sideLabel(side)} 광고`;
    scheduleZone();
  }

  function bindFrame(){
    const frameDoc=doc(),frameWin=win();if(!frameDoc||!frameWin||frameDoc.documentElement.dataset.builderSandbox!=='true')return;
    ...
    if(frameDoc.documentElement.dataset.builderAdZoneBound!=='true'){
      frameDoc.documentElement.dataset.builderAdZoneBound='true';
      frameWin.addEventListener('scroll',scheduleZone,{passive:true});
      frameWin.addEventListener('resize',scheduleZone,{passive:true});
      frameDoc.addEventListener('transitionend',scheduleZone,true);
    }
    scheduleZone();
  }
