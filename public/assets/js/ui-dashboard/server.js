(function(){
  ...
  wrap.innerHTML='<button type="button" id="uiDashConnect">서버 연결</button><button type="button" id="uiDashLoad" disabled>preset 불러오기</button><button type="button" id="uiDashSave" disabled>preset 저장</button><span id="uiDashServerStatus">브라우저 저장</span>';
  topActions.prepend(wrap);

  const dialog=document.createElement('dialog');
  dialog.className='ui-dashboard-server-dialog';
  dialog.innerHTML='<div class="ui-dashboard-server-card"><small>UI PRESETS</small><h2>관리자 서버 연결</h2><p>저장한 UI preset을 Google Sheets와 동기화합니다. 토큰은 이 탭의 세션에만 보관합니다.</p><label>관리자 토큰<input id="uiDashToken" type="password" autocomplete="off" spellcheck="false"></label><span id="uiDashConnectMessage" role="status"></span><div class="ui-dashboard-server-actions"><button type="button" data-ui-dash-cancel>취소</button><button type="button" data-ui-dash-submit>연결</button></div></div>';
  ...
  function syncButtons(){const connected=Boolean(getToken());connect.textContent=connected?'연결됨':'서버 연결';load.disabled=!connected;save.disabled=!connected;}
  ...
  async function loadPresets(){setStatus('preset 불러오는 중');load.disabled=true;save.disabled=true;try{const data=await request('/api/editor/ui-presets');const current=readLocal();const byId=new Map(current.map(item=>[item.id,item]));for(const item of data.presets||[])byId.set(item.id,item);writeLocal([...byId.values()]);setStatus(`preset ${data.count||0}개 불러옴`,'ok');window.location.reload();}catch(error){setStatus(error?.message||'preset을 불러오지 못했습니다.','error');syncButtons();}}

  async function savePresets(){setStatus('preset 저장 중');load.disabled=true;save.disabled=true;try{const presets=readLocal();const data=await request('/api/editor/ui-presets',{method:'POST',body:{presets}});setStatus(`preset ${data.count||presets.length}개 저장됨`,'ok');}catch(error){setStatus(error?.message||'preset을 저장하지 못했습니다.','error');}finally{syncButtons();}}

  connect.addEventListener('click',()=>{if(getToken()){if(window.confirm('이 탭의 관리자 연결을 해제할까요?')){setToken('');syncButtons();setStatus('브라우저 저장');}return;}message.textContent='';dialog.showModal();setTimeout(()=>tokenInput.focus(),0);});
