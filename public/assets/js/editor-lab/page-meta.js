(function(){
  ...
  function uid(type='page'){
    return `${type}_${Date.now().toString(36)}_${Math.random().toString(36).slice(2,7)}`;
  }

  function normalizeId(value){
    return String(value||'').trim().toLowerCase().replace(/[^a-z0-9가-힣_.:-]+/g,'-').replace(/^-+|-+$/g,'').slice(0,120)||'general';
  }
  ...
  industryInput.addEventListener('change',()=>{
    const draft=readDraft();
    draft.industryId=normalizeId(industryInput.value);
    writeDraft(draft);
    sync();
  });
  ...
  slugInput.addEventListener('change',()=>{
    const draft=readDraft();
    draft.slug=normalizeSlug(slugInput.value);
    writeDraft(draft);
    sync();
  });
  ...
  duplicateButton?.addEventListener('click',()=>{
    ...
    draft.blocks=draft.blocks.map(block=>({
      ...block,
      revision:{version:1,updatedAt:new Date().toISOString(),updatedBy:'editor-lab-copy'}
    }));
    writeDraft(draft);
    window.location.reload();
  });
