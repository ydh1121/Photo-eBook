(function(){
  ...
  function setConnected(value){
    ...
    pagesSelect.disabled=!connected;
    loadButton.disabled=!connected||!pagesSelect.value;
    saveButton.disabled=!connected;
    connectButton.textContent=connected?'연결 다시 설정':'서버 연결';
  }
  ...
  function setBusy(value){
    ...
    pagesSelect.disabled=busy||!connected;
  }
  ...
  function readDraft(){
    try{
      ...
      if(!parsed||!Array.isArray(parsed.blocks))throw new Error('브라우저 초안을 찾지 못했습니다.');
      return parsed;
    }catch(error){
      throw new Error(error?.message||'브라우저 초안을 읽지 못했습니다.');
    }
  }
  ...
  function toServerPayload(draft){
    return {
      page:{
        pageId:draft.pageId,
        slug:draft.slug||'',
        industryId:draft.industryId||'general',
        title:draft.pageTitle||'새 분야 가이드',
  ...
  function fromServerPage(page,current={}){
    return {
      ...
      pageId:page.pageId,
      slug:page.slug||'',
      industryId:page.industryId||'general',
      pageTitle:page.title||'새 분야 가이드',
      theme:page.theme==='system'?'light':(page.theme||'light'),
      seo:page.seo||{},
      preview:current.preview||'desktop',
      mode:'edit',
      blocks:Array.isArray(page.blocks)?page.blocks:[],
      serverUpdatedAt:page.updatedAt||null,
  ...
  async function verifyToken(token,{persist=true}={}){
    ...
    setStatus('서버 확인 중');
    try{
      const health=await requestApi('health',{token});
      if(!health.ok)throw new Error('서버 설정을 확인해 주세요.');
      if(persist)setToken(token);
      setConnected(true);
      await refreshPages();
      setStatus('서버 초안 연결됨','connected');
      return true;
    }catch(error){
      if(persist)setToken('');
      setConnected(false);
      setStatus('서버 미연결','error');
      throw error;
    }finally{
      setBusy(false);
    }
  ...
  async function saveServerDraft(){
    ...
    try{
      ...
      writeDraft(draft);
      await refreshPages();
      pagesSelect.value=draft.pageId;
      loadButton.disabled=false;
      setStatus(`서버 저장됨 · ${data.blockCount}개 블록`,'connected');
    }catch(error){
      setStatus(error.message||'서버 저장 실패','error');
    }finally{
      setBusy(false);
    }
  }
  ...
  async function loadServerDraft(){
    const pageId=pagesSelect.value;
    if(!pageId)return;
    setBusy(true);
    setStatus('서버 초안 불러오는 중');
    try{
      const data=await requestApi(`page?id=${encodeURIComponent(pageId)}`);
      if(!data.page)throw new Error('페이지 데이터를 찾지 못했습니다.');
      let current={};
      try{current=JSON.parse(localStorage.getItem(DRAFT_KEY)||'{}')||{};}catch{}
      writeDraft(fromServerPage(data.page,current));
      setStatus('서버 초안 불러옴','connected');
  ...
  function openDialog(){
    if(!dialog)return;
    if(messageNode){messageNode.textContent='';messageNode.className='editor-connect-message';}
  ...
  async function submitConnection(){
    ...
    if(!token){
      if(messageNode){messageNode.textContent='관리자 토큰을 입력하세요.';messageNode.className='editor-connect-message is-error';}
      return;
    }
    if(submitButton)submitButton.disabled=true;
    try{
      await verifyToken(token,{persist:true});
      if(messageNode){messageNode.textContent='연결됐습니다.';messageNode.className='editor-connect-message is-ok';}
      setTimeout(()=>dialog?.close(),250);
    }catch(error){
      if(messageNode){messageNode.textContent=error.message||'연결하지 못했습니다.';messageNode.className='editor-connect-message is-error';}
    }finally{
  ...
  if(token){
    verifyToken(token,{persist:false}).catch(()=>setToken(''));
  }else{
    setStatus('브라우저 초안');
  }
})();
